...
{
	internal class MultiColumnTreeView : TreeViewWithTreeModel<RefTreeElement>
	{
		...
		public MultiColumnTreeView (TreeViewState state, MultiColumnHeader multicolumnHeader, TreeModel<RefTreeElement> model) : base (state, multicolumnHeader, model)
		{
		...
		void SortByMultipleColumns ()
		{
			...
			var myTypes = rootItem.children.Cast<TreeViewItem<RefTreeElement> >();
			...
			for (int i=1; i<sortedColumns.Length; i++)
			{
				...
				switch (sortOption)
				{
					...
					case SortOption.Path:
						...
						break;
					case SortOption.Guid:
						orderedQuery = orderedQuery.ThenBy(l => l.data.guid, ascending);
						break;
		...
		IOrderedEnumerable<TreeViewItem<RefTreeElement>> InitialOrder(IEnumerable<TreeViewItem<RefTreeElement>> myTypes, int[] history)
		{
			...
			switch (sortOption)
			{
				...
				case SortOption.Guid:
					return myTypes.Order(l => l.data.guid, ascending);
				case SortOption.Type:
		...
		int GetIcon1Index(TreeViewItem<RefTreeElement> item)
		{
			return 0;
			// return (int)(Mathf.Min(0.99f, item.data.floatValue1) * s_TestIcons.Length);
		}

		int GetIcon2Index (TreeViewItem<RefTreeElement> item)
		{
			return 1;
		...
		{
			var item = (TreeViewItem<RefTreeElement>) args.item;
		...
		void CellGUI (Rect cellRect, TreeViewItem<RefTreeElement> item, MyColumns column, ref RowGUIArgs args)
		{
				...
				case MyColumns.Value3:
					{
						...
						else
						{
							...
							// MultiColumnHeader.DefaultGUI.LabelRightAligned(cellRect, value, args.selected, args.focused);
						}
					}
					break;
			}
